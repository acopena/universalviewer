...
export class DownloadDialogue extends BaseDownloadDialogue {
      ...
      if (this.renderingUrls[<any>id]) {
        ...
        switch (type) {
          case DownloadOption.CURRENT_VIEW:
            const viewer: any = (<OpenSeadragonExtension>(
              that.extension
            )).getViewer();
            window.open(
              <string>(
                (<OpenSeadragonExtension>that.extension).getCroppedImageUri(
                  canvas,
                  viewer
                )
              )
          ...
          case DownloadOption.WHOLE_IMAGES_HIGH_RES:
            ...
            for (let i = 0; i < indices.length; i++) {
              window.open(
                this.getCanvasHighResImageUri(
                  this.extension.helper.getCanvasByIndex(indices[i])
                )
              );
            }
          ...
          case DownloadOption.WHOLE_IMAGE_LOW_RES:
            const imageUri: string | null = (<OpenSeadragonExtension>(
              that.extension
  ...
  open(triggerButton?: HTMLElement) {
    ...
    if (this.isDownloadOptionAvailable(DownloadOption.CURRENT_VIEW)) {
      ...
      const viewer = (<OpenSeadragonExtension>this.extension).getViewer();
      const dimensions: CroppedImageDimensions | null = (<
        OpenSeadragonExtension
      >this.extension).getCroppedImageDimensions(canvas, viewer);
    ...
    if (this.isDownloadOptionAvailable(DownloadOption.WHOLE_IMAGE_HIGH_RES)) {
      ...
      let mime: string | null = this.getCanvasMimeType(
        this.extension.helper.getCurrentCanvas()
      );
      ...
      // dimensions
      const size: Size | null = this.getCanvasComputedDimensions(
        this.extension.helper.getCurrentCanvas()
      );
    ...
    if (this.isDownloadOptionAvailable(DownloadOption.WHOLE_IMAGES_HIGH_RES)) {
      ...
      const label: string = Strings.format(
        this.content.wholeImagesHighRes,
        mime
      );
      $label.text(label);
      $input.prop("title", label);
    ...
    if (this.isDownloadOptionAvailable(DownloadOption.WHOLE_IMAGE_LOW_RES)) {
      ...
      const size: Size | null = (<OpenSeadragonExtension>(
        this.extension
      )).getConfinedImageDimensions(canvas, this.options.confinedImageSize);
    ...
    if (this.isDownloadOptionAvailable(DownloadOption.CURRENT_VIEW)) {
      const currentWidth: number = parseInt(
        this.$currentViewAsJpgButton.data("width").toString()
      );
      const currentHeight: number = parseInt(
        this.$currentViewAsJpgButton.data("height").toString()
      );
      const wholeWidth: number = parseInt(
        this.$wholeImageHighResButton.data("width").toString()
      );
      const wholeHeight: number = parseInt(
        this.$wholeImageHighResButton.data("height").toString()
      );

      const percentageWidth: number = (currentWidth / wholeWidth) * 100;
      const percentageHeight: number = (currentHeight / wholeHeight) * 100;

      const disabledPercentage: number = this.options
    ...
    $groups.each((index: number, group: Element) => {
      ...
      if (
        $group.find("li.option:hidden").length ===
    ...
    if (
      (<OpenSeadragonExtension>this.extension).isPagingSettingEnabled() &&
  ...
  getCanvasDimensions(canvas: Canvas): Size | null {
    ...
    if (canvas.externalResource.data) {
      const width: number | undefined = (<IExternalImageResourceData>(
        canvas.externalResource.data
      )).width;
      const height: number | undefined = (<IExternalImageResourceData>(
        canvas.externalResource.data
      )).height;
      if (width && height) {
